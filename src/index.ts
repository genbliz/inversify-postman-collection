...
class RouteUtilServiceBase {
  //
  getRoutesFromInversifyRouteDef({
    routesDefs,
    baseUrl,
    title,
    routeDefData,
  ...
  private getValidQueryParams(args?: string[]) {
    ...
    const queryParams = queryParamResult.filter(
      (x) => !x.startsWith(defPrefix)
    );

    const queryParamDef = queryParamResult.filter((x) =>
      x.startsWith(defPrefix)
    );
  ...
  getPostmanCollectionFromInversifyRouteDef({
    routesDefs,
    title,
    baseUrl,
    headerEnvVariables,
...
const RouteUtilService = new RouteUtilServiceBase();

export const getPostmanCollection =
  RouteUtilService.getPostmanCollectionFromInversifyRouteDef;

export const getRouteDefinitions =
  RouteUtilService.getRoutesFromInversifyRouteDef;
